...
# PHASE 2: Once red reaches green, move GREEN square to the right --> # Make the green square on 
# the right move at a speed three times faster than the square on the left
push_steps = 80  # adjust to match video timing

for i in range(push_steps):
    square_2.move((speed*3, 0))
    square_1.present(clear=True, update=False)
    square_2.present(clear=False, update=True)
    exp.clock.wait(frame_duration)
